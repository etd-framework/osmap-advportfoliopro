...
class PlgOSMapAdvportfoliopro extends OSMap\Plugin\Base implements OSMap\Plugin\ContentInterface
{
    ...
    public static function prepareMenuItem($node, &$params)
    {
        ...
        $id   = ArrayHelper::getValue($linkVars, 'id', 1);
        switch ($view) {

            case 'projects':
                $node->adapterName = 'AdvPortfolioProProjects';
                $node->uid         = 'advportfoliopro.projects.' . $id;
            ...
            case 'project':
                ...
                $node->adapterName = 'AdvPortfolioProProject';
                ...
                $query = $db->getQuery(true)
                            ...
                            ->from($db->quoteName('#__advportfoliopro_projects'))
                ...
                if (($item = $db->loadObject()) != null) {
                    // Set the node UID
                    $node->uid = 'advportfoliopro.project.' . $id;
        ...
        switch ($view) {
            ...
            case 'project':
                ...
                if ($paramAddPageBreaks) {
                    ...
                    // // Set the node UID
                    $item->uid = 'advportfoliopro.project.' . $id;

                    $menuItem->slug = $item->alias ? ($id . ':' . $item->alias) : $id;
                    $menuItem->link = AdvPortfolioProHelperRoute::getProjectRoute($menuItem->slug, $item->catid);
    ...
    public static function expandCategory(
        ...
        $where = array(
            'a.parent_id = ' . $catid,
            'a.published = 1',
            'a.extension=' . $db->quote('com_advportfoliopro')
        );
        ...
        if ($curlevel <= $maxLevel) {
            if (count($items) > 0) {
                ...
                foreach ($items as $item) {
                    $node                           = new stdClass;
                    $node->id                       = $item->id;
                    $node->uid                      = 'advportfoliopro.projects.' . $item->id;
                    ...
                    $node->link = AdvPortfolioProHelperRoute::getCategoryRoute($node->slug);
    ...
    public static function includeCategoryContent($collector, $parent, $catid, &$params, $itemid, $prevnode = null)
    {
        ...
        if (count($items) > 0) {
            ...
            foreach ($items as $item) {
                $node                           = new stdClass();
                $node->id                       = $item->id;
                $node->uid                      = 'advportfoliopro.project.' . $item->id;
                $node->browserNav               = $parent->browserNav;
                ...
                $node->link    = AdvPortfolioProHelperRoute::getProjectRoute($node->slug, $node->catslug);
    ...
    private static function printSubNodes($collector, $parent, &$params, &$subnodes, $item)
    {
        ...
        foreach ($subnodes as $subnode) {
            ...
            $subnode->publishUp  = $item->created;
